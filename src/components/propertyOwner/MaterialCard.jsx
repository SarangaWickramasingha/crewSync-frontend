...
export default function MaterialCard({ product, onRequest }) {
  ...
  return (
    <div className="group relative bg-white border border-slate-200/80 rounded-xl p-4 hover:shadow-lg hover:border-[var(--color-owner)]/40 transition-all duration-300 flex flex-col justify-between overflow-hidden">
      {/* Top Accent line */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[var(--color-owner)] to-[var(--color-owner-dark)] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

      <div>
        {/* Header: Icon & Badges */}
        <div className="flex items-start justify-between gap-2.5 mb-3">
          <div className="relative w-14 h-14 rounded-xl bg-gradient-to-br from-[var(--color-owner-light)]/80 to-emerald-50/50 p-2 border border-[var(--color-owner-light)] flex items-center justify-center group-hover:scale-105 transition-transform duration-300 shadow-xs flex-shrink-0">
            <Image
              src={iconPath}
        ...
        {/* Title & Supplier */}
        <h3 className="text-sm font-bold text-slate-900 line-clamp-1 group-hover:text-[var(--color-owner)] transition-colors">
          {name}
        </h3>

        {/* Business Name */}
        {businessName && (
          <p className="text-[11px] font-semibold text-[var(--color-owner)] mt-0.5 flex items-center gap-1 truncate">
        ...
        <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between">
          ...
          <span className="text-base font-bold text-[var(--color-owner-dark)] tracking-normal">
            {price}
          </span>
        </div>
      ...
      {/* Action Button */}
      <button
        className="w-full mt-3.5 bg-[var(--color-owner)] hover:bg-[var(--color-owner-dark)] active:scale-[0.99] text-white text-xs font-bold py-2 rounded-xl shadow-xs transition-all duration-200 cursor-pointer text-center"
